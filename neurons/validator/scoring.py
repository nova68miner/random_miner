...
if __name__ == "__main__":
    # Default locations consistent with the sampler
    sampler_file = os.path.join(os.path.dirname(__file__), "..", "random_sample.json")
    sampler_file = os.path.abspath(sampler_file)
    output_file = os.path.join(os.path.dirname(__file__), "..", "random_sample_scored.json")
    output_file = os.path.abspath(output_file)

    bt.logging.info(f"Scoring sampler molecules from {sampler_file}")
    score_dict = score_molecules_json(sampler_file, output_file, target_proteins, antitarget_proteins, subnet_config)
    bt.logging.info(f"Random sample scores: {score_dict}")
